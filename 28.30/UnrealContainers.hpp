...
namespace UC
{
	...
	public:
		...
		struct rebind
		{
			using other = TMemoryAllocator<U>;
		};

		TMemoryAllocator() noexcept = default;

		template<class U>
		TMemoryAllocator(const TMemoryAllocator<U>&) noexcept {}
		...
		template<class U>
		bool operator==(const TMemoryAllocator<U>&) const noexcept { return true; }

		template<class U>
		bool operator!=(const TMemoryAllocator<U>&) const noexcept { return false; }
	};
